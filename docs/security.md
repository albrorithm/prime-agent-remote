...
- read a diagnostics summary: the gateway's version, Prime Agent's version and install origin (never its path), and whether the daemon is connected. No provider configuration, filesystem path, or daemon detail is included;
